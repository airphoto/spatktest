...
object RecommendByALS {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 字段顺序：用户ID  影片ID  星级  时间戳  只需要取前三个字段
    val rawData = spark.read.textFile("F:\\tmpdata\\test\\ml\\ml-100k\\ml-100k\\u.data")
    //ALS 模型需要一个有Rating记录构成的RDD ，Rating就是对user，product和评分的封装
    val ratings = rawData.map(parseRating(_))
...
//      */
//    val model = ALS.train(ratings.rdd)
//
